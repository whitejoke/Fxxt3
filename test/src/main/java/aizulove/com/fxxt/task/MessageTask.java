 ...
 */
public class MessageTask extends AsyncTask<Void,Void,List<Integration>> {
    ...
    public MessageTask(Context context, List<Integration> listMessage, MessageAdapter adapter, Map<String, String> map, String url) {
    ...
    protected List<Integration> doInBackground(Void... params) {
        ...
        try {
            if (judgeInternet) {
                ...
                result = JsonParserFactory.getMessageListByUsername(jsonStr.toString());
            }
        } catch (Exception e) {
            e.printStackTrace();
            result = null;
